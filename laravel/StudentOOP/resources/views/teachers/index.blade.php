...
@section('content')
    ...
    <div class="container">
            ...
            <div class="col-sm" style="text-align: center;padding-top: 2%">
                <table style="border: 1px solid black;text-align: center;" class="table table-secondary table-sm">
                    ...
                    @foreach($teacher as $th)
                        <tr>
                            <th style="border-right: solid 1px #000;"><a href="/teachers/{{$th->id}}">{{$th->id}}</a>
                            </th>
                            <td>{{$th->degree}}</td>
                            <td>{{$th->firstName}}</td>
                            <td>{{$th->lastName}}</td>
                            ...
                            <td>
                                <form action="/teachers/{{$th->id}}/delete/subject" method="POST">
                                    ...
                                    @foreach($th->subjects as $item)
                                        <button type="submit" id="lddButton" name="subjectId"
                                                value="{{$item->id}}">{{$item->name}}</button>
                                    @endforeach
                                    <a href="teachers/{{$th->id}}/add/subject">
        ...
        <div class="d-flex justify-content-center"
             style="margin-bottom: 15px;margin-top: 5px">{{$teacher->links("pagination::bootstrap-4")}}</div>
    </div>
    <div class="container" style="width: 10%; text-align: center">
        <div class="row">
            <button name="insert" class="btn btn-secondary"><a href="/teachers/create" style="color: white">Create</a>
            </button>
        </div>
    </div>
@endsection
