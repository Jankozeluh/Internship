    ...
    <div class="container">
        <div class="row">
            <div class="col-sm" style="text-align: center;padding-top: 2%">
                ...
                <h5>Garant: <a
                        href="/teachers/{{$subject->garantName->id}}">{{$subject->garantName->degree . " " . $subject->garantName->firstName . " " . $subject->garantName->lastName}}</a>
                </h5>
                <h5>Prerequisites:
            ...
            <div class="col-sm" style="text-align: center;padding-top: 2%">
                <table style="border: 1px solid black;text-align: center;" class="table table-secondary table-sm">
                    ...
                    @foreach($subject->students as $item)
                        <tr>
                            <td style="border-right: solid 1px #000;"><a
                                    href="/students/{{$item->id}}">{{$item->id}}</a></td>
                            <td>
                                <a href="/students/{{$item->id}}">{{$item->degree." ".$item->firstName." ".$item->lastName}}</a>
                            </td>
                            <td>{{$item->credits}}</td>
